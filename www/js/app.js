angular.module("underscore", []).factory("_", function() {
    ...
    e.state("app", {
        ...
        url: "/anexos",
        templateUrl: "views/auth/anexos.html",
        controller: "WelcomeBackCtrl"
    }).state("grupos", {
        url: "/grupos",
        templateUrl: "views/auth/grupos.html",
        controller: "WelcomeBackCtrl"
    }).state("evolucao", {
        url: "/evolucao",
        templateUrl: "views/auth/evolucao.html",
        controller: "WelcomeBackCtrl"
    }).state("submenus", {
        url: "/submenus",
...
alert('SEU CADASTRO FOI ENVIADO COM SUCESSO !');
        ...
        var url = 'http://www.nutrivirtual.com.br/aplicativo/cadastro/?callback=JSON_CALLBACK&nome='+formData['nome']+'&estado='+formData['estado']+'&cidade='+formData['cidade']+'&email='+formData['email']+'&telefone='+formData['telefone']+'&whatsapp='+formData['whatsapp'];
        var script = document.createElement('script');
        script.src = url;
        document.getElementsByTagName('head')[0].appendChild(script);
        e.error = "CADASTRO ENVIADO";
...
}]).controller("WelcomeBackCtrl", ["$scope", "$state", "$ionicModal", "ProfileService", "$sce", function(e, n, t, x, y) {
 

 var dadosUser =x.getUserData();

         dadosUser.then(function(successResponse){
             
          
             e.link = y.trustAsResourceUrl('http://www.nutrivirtual.com.br/webroot/chat_app/index.php?usuario=' + successResponse.dadosUser.users[0].name.first + '&senha=' + successResponse.dadosUser.users[0].email);
             
             console.log('http://www.nutrivirtual.com.br/webroot/mensagens/index.php?usuario=' + successResponse.dadosUser.users[0].name.first + '&senha=' + successResponse.dadosUser.users[0].email);
          });
...
>>>>>>> origin/master
        ...
        e.put("views/app/partials/profile-post-card.html", ''), 
